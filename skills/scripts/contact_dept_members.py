#!/usr/bin/env python3
"""
按部门名称搜索并列出所有成员（自动 groupId 解析）

用法:
    python contact_dept_members.py --ent-id <ent-id> --query "技术部"
    python contact_dept_members.py --ent-id <ent-id> --query "产品" --dry-run
"""
...
def main():
    ...
    parser.add_argument(
        '--ent-id', required=True, help='企业 ID'
    )
    parser.add_argument(
    ...
    dept_data = run_ykc([
        ...
        '--ent-id', args.ent_id,
        '--keyword', args.query, '--format', 'json',
    ...
    for dept in depts:
        dept_id = dept.get('id') or dept.get('groupId')
        dept_name = dept.get('name') or dept.get('groupName', '未知')
        if not dept_id:
            continue
        ...
        members_data = run_ykc([
            'contact', 'group', 'members',
            '--ent-id', args.ent_id,
            '--group-id', str(dept_id), '--format', 'json',
        ])
        if not members_data:
            print('  无法获取成员列表')
            continue
