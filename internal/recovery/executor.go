...
type ToolInvoker interface {
	CallToolDirect(ctx context.Context, serverID, toolName string, args map[string]any) (*ToolCallResult, error)
}
...
func defaultProbes() []Probe {
	return []Probe{probeUnknownContextAudit}
}
...
func summarizeProbeOutput(result *ToolCallResult) (any, string) {
	if result == nil {
...
func buildAgentTask(eventID string, rc RecoveryContext, replay Replay, plan RecoveryPlan, probes []ProbeResult) AgentTask {
	...
	return AgentTask{
		Goal:         "阅读完整 RecoveryBundle，并基于 ykc skill 对 unknown 恢复场景做整体分析后再决定是否发起 grounded 的下一次 ykc 尝试",
		...
		AllowedActions: []string{
			...
			"仅在修复依据明确时重新发起新的 ykc 命令",
		},
		ForbiddenActions: []string{
			"编造 ID、UUID、token、URL 或其他业务参数",
			"绕过 ykc 直接调用 HTTP API、curl 或浏览器",
			"未确认前把失败命令替换成另一套业务流程",
		},
		StopConditions: []string{
			"新的 ykc 命令已经成功并可回写 recovered",
...
func buildFinalizeHint(eventID string) FinalizeHint {
	return FinalizeHint{
		Required:            true,
		Command:             fmt.Sprintf("ykc recovery finalize --event-id %s --outcome <recovered|failed|handoff> --execution-file <file.json> --format json", eventID),
