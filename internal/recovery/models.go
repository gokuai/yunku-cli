...
func BuildContext(input CaptureInput) RecoveryContext {
	...
	// Extract HTTP-related info from wrapped errors if available
	if input.WrappedErr != nil {
		errStr := input.WrappedErr.Error()
		...
		if strings.Contains(errStr, "429") {
			...
			ctx.HTTPStatus = 404
		} else if strings.Contains(errStr, "401") || strings.Contains(errStr, "token") {
			ctx.HTTPStatus = 401
		}
	}
