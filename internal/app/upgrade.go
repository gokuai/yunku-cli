...
import (
	...
	"github.com/gokuai/yunku-cli/internal/upgrade"
...
func newUpgradeCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "upgrade",
		Short: "升级 ykc CLI 到最新版本",
		Long: `检查并升级 ykc CLI 到最新版本。

自动下载匹配当前平台的二进制文件，通过 SHA256 校验后原子替换。
升级前会自动备份当前版本，可通过 --rollback 回滚。`,
		Example: `  ykc upgrade                    # 交互式升级到最新版本
  ykc upgrade --check            # 仅检查是否有新版本
  ykc upgrade --list             # 列出最近版本
  ykc upgrade --list --all       # 列出所有版本
  ykc upgrade --version v1.0.5   # 升级到指定版本
  ykc upgrade --rollback         # 回滚到上一版本
  ykc upgrade -y                 # 跳过确认直接升级`,
		Args: cobra.NoArgs,
...
func runUpgradeList(cmd *cobra.Command, format string, limit int) error {
	...
	fmt.Printf("  %s\n", ugDim("提示: 使用 ykc upgrade --version v1.0.7 安装指定版本"))
	return nil
}

// --- ykc upgrade --rollback ---
...
//   Phase 2 (Apply):   replace binary — only runs if Phase 1 fully succeeds.
...
func runUpgrade(ctx context.Context, opts upgradeOptions) error {
	...
	if err != nil {
		tmpDir, err = os.MkdirTemp("", "ykc-upgrade-*")
		if err != nil {
	...
	// Steps: 1.备份  2.下载  3.校验  4.解压验证  5.替换
	const totalSteps = 5
	...
	// Fetch checksums.txt (needed for strict verification)
	...
	if binaryPath == "" {
		fmt.Println()
		return fmt.Errorf("在解压目录中未找到 ykc 二进制文件")
	}
	...
	fmt.Println()
	fmt.Printf("  %s\n", ugDim("运行 ykc version 验证当前版本"))
	fmt.Printf("  %s\n", ugDim("如遇问题，运行 ykc upgrade --rollback 回滚"))
