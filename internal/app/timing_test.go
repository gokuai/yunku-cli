...
func TestBuildReport(t *testing.T) {
	...
	report := tc.BuildReport("v1.0.8", "ykc contact user search")
	...
	if report.Command != "ykc contact user search" {
		t.Errorf("expected command 'ykc contact user search', got %q", report.Command)
	}
...
func TestBuildReportEmpty(t *testing.T) {
	tc := NewTimingCollector()
	report := tc.BuildReport("dev", "ykc version")
...
func TestBuildReportJSON(t *testing.T) {
	...
	report := tc.BuildReport("v1.0.0", "ykc version")
...
func TestWriteReportIfEnabled(t *testing.T) {
	...
	tc.WriteReportIfEnabled("v1.0.0", "ykc version")
...
func TestWriteReportIfEnabled_Auto(t *testing.T) {
	tmpHome := t.TempDir()
	expected := filepath.Join(tmpHome, ".ykc", "perf", "latest.json")
...
func TestLoadLatestReport(t *testing.T) {
	...
	perfDir := filepath.Join(tmpHome, ".ykc", "perf")
...
func TestSanitizeCommand(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			args: []string{"ykc", "contact", "user", "search"},
			want: "ykc contact user search",
		},
		{
			name: "token with space-separated value",
			args: []string{"ykc", "--token", "secret123", "version"},
			want: "ykc --token *** version",
		},
		{
		...
		},
		{
			...
			args: []string{"ykc", "--client-secret", "mysecret", "--client-id", "myid", "auth"},
			want: "ykc --client-secret *** --client-id *** auth",
		},
		{
			name: "client-id with equals",
			args: []string{"ykc", "--client-id=abc123"},
			want: "ykc --client-id=***",
		},
		{
			name: "empty args",
