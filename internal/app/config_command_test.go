...
func seedTestConfig(t *testing.T) {
	...
	configmeta.Register(configmeta.ConfigItem{
		Name: "YKC_CONFIG_DIR", Category: configmeta.CategoryCore,
		Description: "覆盖默认配置目录", DefaultValue: "~/.ykc",
	})
	configmeta.Register(configmeta.ConfigItem{
		Name: "YKC_CLIENT_SECRET", Category: configmeta.CategoryAuth,
		Description: "OAuth AppSecret", Sensitive: true,
	})
	configmeta.Register(configmeta.ConfigItem{
		Name: "YKC_CATALOG_FIXTURE", Category: configmeta.CategoryDebug,
...
func TestConfigListTable(t *testing.T) {
	...
	out := buf.String()
	if !strings.Contains(out, "YKC_CONFIG_DIR") {
		t.Error("expected YKC_CONFIG_DIR in output")
	}
	if !strings.Contains(out, "YKC_CLIENT_SECRET") {
		t.Error("expected YKC_CLIENT_SECRET in output")
	}
	// Hidden items should be excluded by default
	if strings.Contains(out, "YKC_CATALOG_FIXTURE") {
		t.Error("expected YKC_CATALOG_FIXTURE to be hidden")
	}
}
...
func TestConfigListShowHidden(t *testing.T) {
	...
	out := buf.String()
	if !strings.Contains(out, "YKC_CATALOG_FIXTURE") {
		t.Error("expected YKC_CATALOG_FIXTURE with --show-hidden")
	}
}
...
func TestConfigListCategory(t *testing.T) {
	...
	out := buf.String()
	if !strings.Contains(out, "YKC_CLIENT_SECRET") {
		t.Error("expected YKC_CLIENT_SECRET for auth category")
	}
	if strings.Contains(out, "YKC_CONFIG_DIR") {
		t.Error("YKC_CONFIG_DIR should not appear for auth category")
	}
}
...
func TestConfigListShowValues(t *testing.T) {
	...
	t.Setenv("YKC_CONFIG_DIR", "/custom/dir")
	t.Setenv("YKC_CLIENT_SECRET", "supersecret123")
	...
	if !strings.Contains(out, "/custom/dir") {
		t.Error("expected actual value for YKC_CONFIG_DIR")
	}
