...
func init() {
	...
	configmeta.Register(configmeta.ConfigItem{
		Name:        "YKC_PERF_REPORT",
		Category:    configmeta.CategoryDebug,
		Description: "JSON 性能报告输出路径 (auto=~/.ykc/perf/latest.json)",
		Example:     "auto",
	})
}
...
const (
	...
	PerfDebugEnv = "YKC_PERF_DEBUG"

	// PerfReportEnv is the environment variable to enable JSON perf report output.
	// Set to "auto" to write to ~/.ykc/perf/latest.json, or a custom file path.
	PerfReportEnv = "YKC_PERF_REPORT"
...
// PrintIfEnabled prints timing info to stderr if YKC_PERF_DEBUG is set.
...
// WriteReportIfEnabled checks YKC_PERF_REPORT and writes a JSON report if set.
...
// LoadLatestReport reads the default perf report file (~/.ykc/perf/latest.json).
...
// resolvePerfReportPath resolves the YKC_PERF_REPORT value to an absolute path.
...
func defaultPerfReportPath() string {
	...
	return filepath.Join(home, ".ykc", perfReportDir, perfReportFile)
}
