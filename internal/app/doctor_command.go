...
func doctorCheckAuth(_ context.Context, w io.Writer, jsonOut bool) checkResult {
	if !jsonOut {
		fmt.Fprint(w, "检查登录状态...       ")
	}
	...
	if data.RefreshToken != "" {
		r := checkResult{
			Name:    "auth",
			Status:  statusWarn,
			Message: "Access Token 已过期, 但 Refresh Token 可用",
			Hint:    "运行 ykc auth refresh 刷新登录状态",
		}
		if !jsonOut {
			printCheckResult(w, r)
		}
...
func doctorCheckNetwork(ctx context.Context, w io.Writer, jsonOut bool, timeout time.Duration) checkResult {
	...
	r := checkResult{
		Name:    "network",
		Status:  statusPass,
		Message: fmt.Sprintf("%s 可达 (延迟 %dms, 状态码 %d)", baseURL, latency.Milliseconds(), resp.StatusCode),
	}
	if !jsonOut {
		printCheckResult(w, r)
	}
...
func doctorCheckCache(w io.Writer, jsonOut bool) checkResult {
	...
	if staleCount > 0 {
		r := checkResult{
			...
			Hint:    "运行 ykc cache refresh 刷新缓存",
		}
		if !jsonOut {
			printCheckResult(w, r)
		}
...
func doctorCheckPerf(w io.Writer, jsonOut bool) checkResult {
	...
	if err != nil {
		r := checkResult{
			...
			Status:  statusWarn,
			Message: "未找到性能报告",
			Hint:    "设置 YKC_PERF_REPORT=auto 后运行任意命令生成报告",
		}
		if !jsonOut {
			printCheckResult(w, r)
		}
