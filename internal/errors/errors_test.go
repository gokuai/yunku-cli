...
func TestPrintJSON(t *testing.T) {
	...
	if err := PrintJSON(&b, NewValidation(
		...
		WithRetryable(true),
		WithActions("ykc schema doc.create_document", "retry command"),
		WithSnapshot("/tmp/ykc-recovery/snapshot.json"),
	)); err != nil {
	...
	if !strings.Contains(got, "\"snapshot_path\": \"/tmp/ykc-recovery/snapshot.json\"") {
...
func TestPrintHuman(t *testing.T) {
	...
	if err := PrintHumanAt(&b, NewValidation(
		"bad flag",
		WithReason("missing_required_flag"),
		WithOperation("contact.user.get"),
		WithServerKey("contact"),
		WithHint("Pass the required flag and retry."),
		WithRetryable(true),
	...
	if !strings.Contains(got, "Snapshot: /tmp/ykc-recovery/snapshot.json") {
