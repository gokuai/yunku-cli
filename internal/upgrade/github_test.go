...
func TestFetchLatestRelease(t *testing.T) {
	release := GitHubRelease{
		...
		Assets: []GitHubAsset{
			{Name: "ykc-darwin-arm64.tar.gz", Size: 3942308, BrowserDownloadURL: "https://example.com/ykc-darwin-arm64.tar.gz"},
			{Name: "checksums.txt", Size: 534, BrowserDownloadURL: "https://example.com/checksums.txt"},
		},
	}

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/repos/gokuai/yunku-cli/releases/latest" {
	...
	if len(info.Assets) != 2 {
		t.Errorf("len(Assets) = %d, want 2", len(info.Assets))
	}
}
...
func TestFetchReleaseByTag(t *testing.T) {
	release := GitHubRelease{
		...
		Assets: []GitHubAsset{
			{Name: "ykc-darwin-arm64.tar.gz", Size: 3800000, BrowserDownloadURL: "https://example.com/ykc-darwin-arm64.tar.gz"},
		},
	}

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		expected := "/repos/gokuai/yunku-cli/releases/tags/v1.0.5"
...
func TestFindBinaryAsset(t *testing.T) {
	assets := []GitHubAsset{
		...
		{Name: "ykc-darwin-amd64.tar.gz", Size: 4276643},
		{Name: "ykc-darwin-arm64.tar.gz", Size: 3942308},
		{Name: "ykc-linux-amd64.tar.gz", Size: 4394696},
		{Name: "ykc-windows-amd64.zip", Size: 4500000},
	}
	...
	tests := []struct {
		...
		{"darwin", "arm64", "ykc-darwin-arm64.tar.gz", false},
		{"darwin", "amd64", "ykc-darwin-amd64.tar.gz", false},
		{"linux", "amd64", "ykc-linux-amd64.tar.gz", false},
		{"windows", "amd64", "ykc-windows-amd64.zip", false},
		{"freebsd", "amd64", "", true},
	}
...
func TestFetchLatestRelease_FieldMapping(t *testing.T) {
	release := GitHubRelease{
		...
		HTMLURL:     "https://github.com/gokuai/yunku-cli/releases/tag/v2.1.0",
		Assets:      []GitHubAsset{},
	}
...
func TestFindChecksumsAsset_NotFound(t *testing.T) {
	assets := []GitHubAsset{{Name: "ykc-darwin-arm64.tar.gz"}}
