// Package upgrade provides self-update functionality for the ykc CLI
...
const (
	...
	defaultOwner  = "gokuai"
	defaultRepo   = "yunku-cli"
	httpTimeout   = 30 * time.Second
	userAgent     = "YKC-Upgrade/1.0"
...
func NewClient() *Client {
	...
	if env := os.Getenv("YKC_UPGRADE_URL"); env != "" {
...
// Pattern: ykc-{os}-{arch}.tar.gz (or .zip for windows).
...
func FindBinaryAssetFor(assets []GitHubAsset, goos, goarch string) (*GitHubAsset, error) {
	...
	target := fmt.Sprintf("ykc-%s-%s%s", goos, goarch, ext)
