...
func TestExtractZip_BasicFiles(t *testing.T) {
	...
	assertFileContent(t, filepath.Join(targetDir, "ykc", "SKILL.md"), "# Skill")
	assertFileContent(t, filepath.Join(targetDir, "ykc", "data.txt"), "some data")
}
...
func TestFindBinaryInDir_TopLevel(t *testing.T) {
	dir := t.TempDir()
	ykcPath := filepath.Join(dir, "ykc")
	os.WriteFile(ykcPath, []byte("#!/bin/sh"), 0755)

	got := FindBinaryInDir(dir)
	if got != ykcPath {
		t.Errorf("FindBinaryInDir() = %q, want %q", got, ykcPath)
	}
}
...
func TestFindBinaryInDir_Nested(t *testing.T) {
	...
	ykcPath := filepath.Join(nested, "ykc")
	os.WriteFile(ykcPath, []byte("#!/bin/sh"), 0755)
...
func TestFindBinaryInDir_Windows(t *testing.T) {
	dir := t.TempDir()
	exePath := filepath.Join(dir, "ykc.exe")
