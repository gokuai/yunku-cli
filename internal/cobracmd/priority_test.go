...
func TestSetOverridePriority(t *testing.T) {
	...
	t.Run("sets priority on cmd with nil annotations", func(t *testing.T) {
		...
		if got := cmd.Annotations["ykc.override-priority"]; got != "10" {
	...
	t.Run("overwrites existing priority", func(t *testing.T) {
		...
		if got := cmd.Annotations["ykc.override-priority"]; got != "7" {
...
func TestOverridePriority(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			cmd: &cobra.Command{
				Use:         "test",
				Annotations: map[string]string{"ykc.override-priority": ""},
			},
			want: 0,
		},
		{
		...
		},
		{
			...
			cmd: &cobra.Command{
				Use:         "test",
				Annotations: map[string]string{"ykc.override-priority": "abc"},
			},
			want: 0,
		},
		{
