...
var preferredListKeys = []string{
	"items", "results", "data", "list", "records", "value",
	"tools", "servers", "products",
	"nodes", "devices", "favorites", "members", "groups", "orgs",
	"activities", "logs", "links",
}
...
func writeRaw(w io.Writer, payload any) error {
	...
	_, err := fmt.Fprint(w, SanitizeForTerminal(strings.TrimRight(buf.String(), "\n")))
	fmt.Fprintln(w)
	return err
}
