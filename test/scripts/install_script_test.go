...
func TestInstallScriptSourceModeInstallsBinary(t *testing.T) {
	...
	// Stub make: when invoked as "make -C <dir> build", create a fake ykc binary
...
`
	...
	cmd.Env = append(os.Environ(),
		...
		"YKC_INSTALL_DIR="+installDir,
		"YKC_INSTALL_NAME=ykc-test",
	)
	output, err := cmd.CombinedOutput()

	// Clean up the fake binary created in the real repo root
	_ = os.Remove(filepath.Join(repoRoot, "ykc"))
	...
	for _, want := range []string{
		"Installing ykc from source checkout",
	...
	binaryPath := filepath.Join(installDir, "ykc-test")
...
func TestInstallScriptsUseGitHubReleaseSkillsAsset(t *testing.T) {
	...
	for _, rel := range []string{
		...
		text := string(data)
		if !strings.Contains(text, "releases/download") || !strings.Contains(text, "ykc-skills.zip") {
			t.Fatalf("%s should download ykc-skills.zip from GitHub Releases", scriptPath)
		}
