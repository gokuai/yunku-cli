#!/usr/bin/env python3
"""
yunku Skill Test Runner
Validates AI Agent's ability to translate natural language prompts into ykc CLI commands.
"""
...
def parse_test_cases(content: str) -> list[TestCase]:
    ...
    while i < len(lines):
        ...
        # Detect command section (#### `ykc command`)
        if line.startswith('#### `ykc '):
            match = re.match(r'#### `ykc (.+)`', line)
            if match:
...
def generate_report(results: list[dict]) -> str:
    ...
    report = []
    report.append("# yunku Skill Test Results\n")
