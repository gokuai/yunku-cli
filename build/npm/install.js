...
const PLATFORM_MAP = {
  "darwin-x64": "ykc-darwin-amd64.tar.gz",
  "darwin-arm64": "ykc-darwin-arm64.tar.gz",
  "linux-x64": "ykc-linux-amd64.tar.gz",
  "linux-arm64": "ykc-linux-arm64.tar.gz",
  "win32-x64": "ykc-windows-amd64.zip",
  "win32-arm64": "ykc-windows-arm64.zip",
};
...
function findBinary(root) {
  ...
  for (const entry of entries) {
    ...
    if (entry.name === "ykc" || entry.name === "ykc.exe") {
      return entryPath;
    }
  }
  return "";
...
function extractArchive(archivePath, destDir) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "ykc-npm-bin-"));
  try {
    ...
    if (!binaryPath) {
      throw new Error(`ykc binary not found in archive ${archivePath}`);
    }

    ensureCleanDir(destDir);
    const targetName = process.platform === "win32" ? "ykc.exe" : "ykc";
...
function installSkillsToHomes(skillRoot) {
  ...
  AGENT_DIRS.forEach((agentDir, index) => {
    ...
    const destDir = path.join(baseDir, "ykc");
  ...
  if (installed === 0) {
    copyChildren(skillRoot, path.join(homeDir, ".agents", "skills", "ykc"));
  }
}
...
function main() {
  ...
  const skillDir = path.join(packageRoot, "share", "skills", "ykc");
  ...
  const skillsPath = path.join(assetsDir, "ykc-skills.zip");
