...
#   YKC_SKILLS_ONLY   — set to 1 to install only skills (skip binary)
#
...
BIN_NAME="ykc"
INSTALL_DIR="${YKC_INSTALL_DIR:-$HOME/.local/bin}"
INSTALL_NAME="${YKC_INSTALL_NAME:-$BIN_NAME}"
VERSION="${YKC_VERSION:-latest}"
NO_SKILLS="${YKC_NO_SKILLS:-0}"
SKILLS_ONLY="${YKC_SKILLS_ONLY:-0}"
SKILL_NAME="ykc"
...
resolve_version() {
  if [ "$VERSION" = "latest" ]; then
    ...
    if [ -z "$VERSION" ]; then
      err "Could not determine the latest version. Set YKC_VERSION explicitly."
    fi
  fi
}
...
print_banner() {
  printf '\n'
  say "┌──────────────────────────────────────┐"
  say "│     Yunku CLI Installer                │"
  say "│     github.com/gokuai/yunku-cli        │"
  say "└──────────────────────────────────────┘"
  printf '\n'
}
...
install_binary_from_source() {
  ...
  say "Installing ykc from source checkout: ${root}"
  say "Install dir: ${INSTALL_DIR}"

  # Build using make (produces ./ykc in the project root)
  make -C "$root" build
...
main() {
  ...
  if [ "$SKILLS_ONLY" != "1" ]; then
    ...
    say "  ${BIN_NAME} auth login       # authenticate with yunku"
  fi
