...
# Install yunku agent skills from GitHub Releases into agent skill directories.
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/gokuai/yunku-cli/main/scripts/install-skills.sh | sh
#
# Downloads ykc-skills.zip from GitHub Releases and copies it under each target
...
# directory. Set YKC_SKILLS_ROOT=$HOME to match npm install layout exactly.
#
# Environment variables (optional):
#   YKC_VERSION        — release tag (default: latest)
#   YKC_SKILLS_ROOT    — base path for agent dirs (default: $PWD)

REPO="gokuai/yunku-cli"
VERSION="${YKC_VERSION:-latest}"
SKILL_NAME="ykc"
ROOT="${YKC_SKILLS_ROOT:-$PWD}"
...
resolve_version() {
  if [ "$VERSION" = "latest" ]; then
    ...
    if [ -z "$VERSION" ]; then
      printf '❌ Could not determine the latest version. Set YKC_VERSION explicitly.\n' >&2
      exit 1
    fi
  fi
}
...
# Same semantics as build/npm/install.js installSkillsToHomes (root = YKC_SKILLS_ROOT or PWD).
...
main() {
  ...
  printf '\n'
  printf '  ┌──────────────────────────────────────┐\n'
  printf '  │     Yunku CLI Skill Installer          │\n'
  printf '  │     github.com/gokuai/yunku-cli        │\n'
  printf '  └──────────────────────────────────────┘\n'
  printf '\n'
  ...
  ASSET_URL="https://github.com/${REPO}/releases/download/${VERSION}/ykc-skills.zip"
  printf '  ⬇  Downloading skills from GitHub Releases: %s (%s)\n' "$REPO" "$VERSION"
  curl -fsSL "$ASSET_URL" -o "$TMPDIR_WORK/ykc-skills.zip"
  extract_zip "$TMPDIR_WORK/ykc-skills.zip" "$TMPDIR_WORK/extracted"
  ...
  printf '\n'
  printf '  ⚡ Requires: ykc CLI installed and on $PATH\n'
