# Installer for ykc (Yunku CLI) on Windows.
...
#   powershell -NoExit -ExecutionPolicy Bypass -Command "irm https://raw.githubusercontent.com/gokuai/yunku-cli/main/scripts/install.ps1 | iex"
#
# Environment variables (all optional):
#   YKC_INSTALL_DIR   — where to put the binary       (default: ~/.local/bin)
#   YKC_VERSION       — version to install             (default: latest)
#   YKC_ARCH          — architecture override          (amd64 or arm64)
#   YKC_NO_SKILLS     — set to 1 to skip skills install
#   YKC_SKILLS_ONLY   — set to 1 to install only skills
#
# Agent skills paths follow build/npm/install.js AGENT_DIRS (order and entries must match).

$ErrorActionPreference = "Stop"

$Repo = "gokuai/yunku-cli"
$BinName = "ykc"
$InstallDir = if ($env:YKC_INSTALL_DIR) { $env:YKC_INSTALL_DIR } else { Join-Path $HOME ".local\bin" }
$Version = if ($env:YKC_VERSION) { $env:YKC_VERSION } else { "latest" }
$NoSkills = $env:YKC_NO_SKILLS -eq "1"
$SkillsOnly = $env:YKC_SKILLS_ONLY -eq "1"
$SkillName = "ykc"
...
function Get-Arch {
    ...
    if ($env:YKC_ARCH) {
        $override = $env:YKC_ARCH.ToLower()
        if ($override -eq "amd64" -or $override -eq "arm64") {
            return $override
        }
        Write-Err "Invalid YKC_ARCH value '$env:YKC_ARCH'. Must be 'amd64' or 'arm64'."
    }
    ...
    Write-Err "Unsupported architecture: Could not detect system architecture. Please set YKC_ARCH environment variable to 'amd64' or 'arm64'."
}
...
function Resolve-LatestVersion {
    if ($Version -eq "latest") {
        ...
        Write-Err "Could not determine the latest version. Set `$env:YKC_VERSION explicitly."
    }
}
...
function Write-Banner {
    Write-Host ""
    Write-Say "┌──────────────────────────────────────┐"
    Write-Say "│     Yunku CLI Installer                │"
    Write-Say "│     github.com/gokuai/yunku-cli        │"
    Write-Say "└──────────────────────────────────────┘"
    Write-Host ""
}
...
function Install-Binary {
    ...
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "ykc-install-$PID"
...
function Install-BinaryFromSource {
    ...
    Write-Say "Installing ykc from source checkout: $Root"
    ...
    $tmpBin = Join-Path ([System.IO.Path]::GetTempPath()) "ykc-build-$PID.exe"
    try {
...
function Install-Skills {
    ...
    $zipUrl = "https://github.com/$Repo/releases/download/$Version/ykc-skills.zip"

    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "ykc-skills-$PID"
    New-Item -ItemType Directory -Path $tmpDir -Force | Out-Null
...
if (!$SkillsOnly) {
    ...
    Write-Say "  $BinName auth login       # authenticate with yunku"
}
